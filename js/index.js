...
/* Recherche et filtres de la FAQ publique */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateFaq() {
    ...
    categories.forEach((category) => {
      ...
      category.querySelectorAll('[data-faq-item]').forEach((item) => {
        const matches = categoryMatches && (!query || normalize(item.dataset.faqSearch).includes(query));
        item.hidden = !matches;
        if (!matches) item.open = false;
        if (matches) { visibleTotal += 1; categoryVisible += 1; }
      });
      category.hidden = categoryVisible === 0;
    });
    if (count) count.textContent = `${visibleTotal} réponse${visibleTotal > 1 ? 's' : ''} affichée${visibleTotal > 1 ? 's' : ''}`;
