...
async function deleteWorker(){ const w=getWorker(); if(!w||!await BastUI.confirm(`Supprimer définitivement la fiche de ${workerName(w)} et toutes ses données ?`,{type:'danger',title:'Supprimer la fiche'}))return; data.workers=data.workers.filter(x=>x.id!==w.id);selectedWorkerId='';saveLocal(`${workerName(w)} : fiche supprimée`);closeModal();renderAll(); }
